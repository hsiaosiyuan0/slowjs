...
int JS_AddModuleExport(JSContext *ctx, JSModuleDef *m,
...
static JSModuleDef *
js_host_resolve_imported_module_atom(JSContext *ctx, JSAtom base_module_name,
                                     JSAtom module_name1) {
...
static int add_resolve_entry(JSContext *ctx, JSResolveState *s, JSModuleDef *m,
                             JSAtom name) {
  JSResolveEntry *re;
...
static JSResolveResultEnum
js_resolve_export1(JSContext *ctx, JSModuleDef **pmodule, JSExportEntry **pme,
                   JSModuleDef *m, JSAtom export_name, JSResolveState *s) {
  JSExportEntry *me;
...
static JSResolveResultEnum
js_resolve_export(JSContext *ctx, JSModuleDef **pmodule, JSExportEntry **pme,
...
static JSValue js_build_module_ns(JSContext *ctx, JSModuleDef *m) {
  ...
  for (i = 0; i < s->exported_names_count; i++) {
    ...
    case EXPORTED_NAME_NS:
      ...
      if (JS_DefineAutoInitProperty(ctx, obj, en->export_name,
                                    JS_AUTOINIT_ID_MODULE_NS, en->u.module,
  ...
  JS_DefinePropertyValue(ctx, obj, JS_ATOM_Symbol_toStringTag,
                         JS_AtomToString(ctx, JS_ATOM_Module), 0);
...
static JSValue js_dynamic_import_job(JSContext *ctx, int argc,
                                     JSValueConst *argv) {
