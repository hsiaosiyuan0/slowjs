...
static JSValue js_proxy_get(JSContext *ctx, JSValueConst obj, JSAtom atom,
  ...
  if (res) {
    if ((desc.flags &
         (JS_PROP_GETSET | JS_PROP_CONFIGURABLE | JS_PROP_WRITABLE)) == 0) {
      if (!js_same_value(ctx, desc.value, ret)) {
        goto fail;
      }
    } else if ((desc.flags & (JS_PROP_GETSET | JS_PROP_CONFIGURABLE)) ==
               JS_PROP_GETSET) {
...
static int js_proxy_set(JSContext *ctx, JSValueConst obj, JSAtom atom,
  ...
  if (!s)
    return -1;
  if (JS_IsUndefined(method)) {
    return JS_SetPropertyGeneric(ctx, s->target, atom, JS_DupValue(ctx, value),
                                 receiver, flags);
  }
  atom_val = JS_AtomToValue(ctx, atom);
  if (JS_IsException(atom_val)) {
  ...
  if (JS_IsUndefined(trap_result_obj)) {
    ...
    if (!(result_desc.flags & JS_PROP_CONFIGURABLE)) {
      if (!target_desc_ret || (target_desc.flags & JS_PROP_CONFIGURABLE))
        goto fail1;
      if ((result_desc.flags & (JS_PROP_GETSET | JS_PROP_WRITABLE)) == 0 &&
  ...
  if (!res) {
    ...
    if (flags & (JS_PROP_HAS_GET | JS_PROP_HAS_SET)) {
      ...
      if ((desc.flags & (JS_PROP_CONFIGURABLE | JS_PROP_WRITABLE)) ==
        ...
        goto fail1;
      } else if ((desc.flags & (JS_PROP_CONFIGURABLE | JS_PROP_WRITABLE)) ==
                     0 &&
...
static int js_proxy_delete_property(JSContext *ctx, JSValueConst obj,
  ...
  if (res) {
    JSPropertyDescriptor desc;
    res2 = JS_GetOwnPropertyInternal(ctx, &desc, JS_VALUE_GET_OBJ(s->target),
                                     atom);
    if (res2 < 0)
      return -1;
    if (res2) {
...
static int js_proxy_get_own_property_names(JSContext *ctx,
  ...
  if (JS_IsUndefined(method)) {
    return JS_GetOwnPropertyNamesInternal(
        ctx, ptab, plen, JS_VALUE_GET_OBJ(s->target),
  ...
  if (JS_GetOwnPropertyNamesInternal(ctx, &tab2, &len2,
  ...
  if (!is_extensible) {
    ...
    for (i = 0; i < len; i++) {
      if (!tab[i].is_enumerable) {
        JS_ThrowTypeError(ctx, "proxy: property not present in target were "
                               "returned by non extensible proxy");
        goto fail;
      }
    }
  }
...
static JSValue js_proxy_revoke(JSContext *ctx, JSValueConst this_val, int argc,
...
void JS_AddIntrinsicProxy(JSContext *ctx) {
  ...
  JS_DefinePropertyValueStr(ctx, ctx->global_obj, "Proxy", obj1,
