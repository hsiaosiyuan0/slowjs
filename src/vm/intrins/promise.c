...
static void
js_promise_resolve_function_free_resolved(JSRuntime *rt,
...
static JSValue js_promise_executor(JSContext *ctx, JSValueConst this_val,
...
static JSValue js_new_promise_capability(JSContext *ctx,
  ...
  if (JS_IsUndefined(ctor)) {
    ...
    result_promise =
        JS_CallConstructor(ctx, ctor, 1, (JSValueConst *)&executor);
  }
...
remainingElementsCount_add(JSContext *ctx, JSValueConst resolve_element_env,
                           int addend) {
  JSValue val;
...
static JSValue js_promise_all_resolve_element(JSContext *ctx,
  ...
  if (resolve_type == PROMISE_MAGIC_allSettled) {
    ...
    if (JS_IsException(str))
      goto fail1;
    if (JS_DefinePropertyValue(ctx, obj, JS_ATOM_status, str, JS_PROP_C_W_E) <
        0)
      goto fail1;
...
static JSValue js_promise_all(JSContext *ctx, JSValueConst this_val, int argc,
                              JSValueConst *argv, int magic) {
  ...
  fail_reject:
    ...
    if (JS_DefinePropertyValueUint32(
            ctx, resolve_element_env, 0, JS_NewInt32(ctx, 1),
    ...
    for (;;) {
      ...
      resolve_element = JS_NewCFunctionData(ctx, js_promise_all_resolve_element,
                                            1, magic, 5, resolve_element_data);
      ...
      if (magic == PROMISE_MAGIC_allSettled) {
        ...
        if (JS_DefinePropertyValueUint32(ctx, values, index, JS_UNDEFINED,
                                         JS_PROP_C_W_E) < 0)
          goto fail_reject1;
    ...
    if (is_zero) {
      ...
      ret = JS_Call(ctx, resolving_funcs[is_promise_any], JS_UNDEFINED, 1,
                    (JSValueConst *)&values);
      if (check_exception_free(ctx, ret))
        goto fail_reject;
    }
...
static JSValue js_promise_race(JSContext *ctx, JSValueConst this_val, int argc,
                               JSValueConst *argv) {
  JSValue result_promise, resolving_funcs[2], item, next_promise, ret;
...
__exception int perform_promise_then(JSContext *ctx, JSValueConst promise,
  ...
  if (s->promise_state == JS_PROMISE_PENDING) {
    ...
    if (s->promise_state == JS_PROMISE_REJECTED && !s->is_handled) {
      ...
      if (rt->host_promise_rejection_tracker) {
        rt->host_promise_rejection_tracker(
            ctx, promise, s->promise_result, TRUE,
            rt->host_promise_rejection_tracker_opaque);
      }
    }
...
static JSValue js_promise_catch(JSContext *ctx, JSValueConst this_val, int argc,
                                JSValueConst *argv) {
  JSValueConst args[2];
  args[0] = JS_UNDEFINED;
  args[1] = argv[0];
...
static JSValue js_promise_then_finally_func(JSContext *ctx,
  ...
  ret =
      JS_InvokeFree(ctx, promise, JS_ATOM_then, 1, (JSValueConst *)&then_func);
...
static JSValue js_promise_finally(JSContext *ctx, JSValueConst this_val,
  ...
  if (!JS_IsFunction(ctx, onFinally)) {
    ...
    for (i = 0; i < 2; i++) {
      then_funcs[i] = JS_NewCFunctionData(ctx, js_promise_then_finally_func, 1,
                                          i, 2, func_data);
...
void JS_AddIntrinsicPromise(JSContext *ctx) {
  ...
  JS_SetPropertyFunctionList(ctx, obj1, js_promise_funcs,
  ...
  obj1 = JS_NewCFunction3(
      ctx, (JSCFunction *)js_function_constructor, "AsyncFunction", 1,
      JS_CFUNC_constructor_or_func_magic, JS_FUNC_ASYNC, ctx->function_ctor);
  JS_SetPropertyFunctionList(ctx, ctx->class_proto[JS_CLASS_ASYNC_FUNCTION],
  ...
  ctx->class_proto[JS_CLASS_ASYNC_GENERATOR] =
      JS_NewObjectProto(ctx, ctx->async_iterator_proto);
  JS_SetPropertyFunctionList(ctx, ctx->class_proto[JS_CLASS_ASYNC_GENERATOR],
