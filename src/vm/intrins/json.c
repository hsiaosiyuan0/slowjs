  ...
  if (JS_IsObject(val)) {
    ...
    if (is_array) {
      ...
      ret = JS_GetOwnPropertyNamesInternal(
          ctx, &atoms, &len, JS_VALUE_GET_OBJ(val),
...
static JSValue js_json_parse(JSContext *ctx, JSValueConst this_val, int argc,
                             JSValueConst *argv) {
  JSValue obj, root;
...
static int js_json_to_str(JSContext *ctx, JSONStringifyContext *jsc,
  ...
  case JS_TAG_OBJECT:
    ...
    indent1 = JS_ConcatString(ctx, JS_DupValue(ctx, indent),
    ...
    if (ret) {
      ...
      if (!JS_IsUndefined(jsc->property_list))
        ...
        tab = js_object_keys(ctx, JS_UNDEFINED, 1, (JSValueConst *)&val,
...
static const JSCFunctionListEntry js_json_obj[] = {
    JS_OBJECT_DEF("JSON", js_json_funcs, countof(js_json_funcs),
                  JS_PROP_WRITABLE | JS_PROP_CONFIGURABLE),
};

void JS_AddIntrinsicJSON(JSContext *ctx) {
  /* add JSON as autoinit object */
  JS_SetPropertyFunctionList(ctx, ctx->global_obj, js_json_obj,
                             countof(js_json_obj));
}