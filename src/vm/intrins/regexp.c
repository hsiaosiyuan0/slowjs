...
static JSValue js_regexp_get_flag(JSContext *ctx, JSValueConst this_val,
                                  int mask) {
  JSRegExp *re;
  int flags;
...
static JSValue js_regexp_exec(JSContext *ctx, JSValueConst this_val, int argc,
                              JSValueConst *argv) {
  JSRegExp *re = js_get_regexp(ctx, this_val, TRUE);
  JSString *str;
  ...
  if (ret != 1) {
    ...
    if (JS_DefinePropertyValue(ctx, obj, JS_ATOM_groups, groups, prop_flags) <
        0)
      goto fail;
    if (JS_DefinePropertyValue(
      ...
      goto fail;
    if (JS_DefinePropertyValue(ctx, obj, JS_ATOM_input, str_val, prop_flags) <
        0)
      goto fail1;
  }
  js_free(ctx, capture);
...
static JSValue JS_RegExpDelete(JSContext *ctx, JSValueConst this_val,
                               JSValueConst arg) {
  JSRegExp *re = js_get_regexp(ctx, this_val, TRUE);
  JSString *str;
  JSValue str_val, val;
...
static JSValue js_regexp_test(JSContext *ctx, JSValueConst this_val, int argc,
                              JSValueConst *argv) {
  JSValue val;
  BOOL ret;
...
static JSValue js_regexp_Symbol_replace(JSContext *ctx, JSValueConst this_val,
  ...
  for (j = 0; j < results->len; j++) {
    ...
    if (JS_IsException(namedCaptures))
      goto exception;
    if (functionalReplace) {
      if (JS_DefinePropertyValueInt64(ctx, tab, n++, JS_NewInt32(ctx, position),
                                      JS_PROP_C_W_E | JS_PROP_THROW) < 0)
        goto exception;
      if (JS_DefinePropertyValueInt64(ctx, tab, n++, JS_DupValue(ctx, str),
                                      JS_PROP_C_W_E | JS_PROP_THROW) < 0)
        goto exception;
...
static JSValue js_regexp_Symbol_split(JSContext *ctx, JSValueConst this_val,
  ...
  while (q < size) {
    if (JS_SetProperty(ctx, splitter, JS_ATOM_lastIndex, JS_NewInt32(ctx, q)) <
        0)
      goto exception;
    JS_FreeValue(ctx, z);
    z = JS_RegExpExec(ctx, splitter, str);
...
static const JSCFunctionListEntry js_regexp_string_iterator_proto_funcs[] = {
    ...
    JS_PROP_STRING_DEF("[Symbol.toStringTag]", "RegExp String Iterator",
...
void JS_AddIntrinsicRegExp(JSContext *ctx) {
  ...
  JS_SetPropertyFunctionList(ctx, ctx->class_proto[JS_CLASS_REGEXP],
  ...
  JS_SetPropertyFunctionList(ctx, obj, js_regexp_funcs,
