...
static __exception int JS_DefinePropertyDesc(JSContext *ctx, JSValueConst obj,
  ...
  ret = JS_DefineProperty(ctx, obj, prop, d.value, d.getter, d.setter,
                          d.flags | flags);
  js_free_desc(ctx, &d);
  return ret;
}
...
static JSValue js_object_create(JSContext *ctx, JSValueConst this_val, int argc,
                                JSValueConst *argv) {
  ...
  if (JS_VALUE_GET_TAG(obj) == JS_TAG_OBJECT) {
    ...
    if (res) {
      ...
      if (desc.flags & JS_PROP_GETSET) {
        if (JS_DefinePropertyValue(ctx, ret, JS_ATOM_get,
                                   JS_DupValue(ctx, desc.getter), flags) < 0 ||
            JS_DefinePropertyValue(ctx, ret, JS_ATOM_set,
                                   JS_DupValue(ctx, desc.setter), flags) < 0)
          goto exception1;
      } else {
        if (JS_DefinePropertyValue(ctx, ret, JS_ATOM_value,
                                   JS_DupValue(ctx, desc.value), flags) < 0 ||
...
static JSValue js_object_hasOwn(JSContext *ctx, JSValueConst this_val, int argc,
                                JSValueConst *argv) {
  JSValue obj;
  JSAtom atom;
  JSObject *p;
...
static JSValue js_object_assign(JSContext *ctx, JSValueConst this_val, int argc,
                                JSValueConst *argv) {
...
JSValue js_object_seal(JSContext *ctx, JSValueConst this_val, int argc,
  ...
  for (i = 0; i < len; i++) {
    ...
    if (JS_DefineProperty(ctx, obj, prop, JS_UNDEFINED, JS_UNDEFINED,
                          JS_UNDEFINED, desc_flags) < 0)
      goto exception;
  }
  js_free_prop_enum(ctx, props, len);
  return JS_DupValue(ctx, obj);
...
static JSValue js_object_is(JSContext *ctx, JSValueConst this_val, int argc,
                            JSValueConst *argv) {
...
const JSCFunctionListEntry js_object_funcs[] = {
    ...
    JS_CFUNC_MAGIC_DEF("entries", 1, js_object_keys,
    ...
    JS_CFUNC_MAGIC_DEF("getOwnPropertyDescriptor", 2,
                       js_object_getOwnPropertyDescriptor, 0),
    JS_CFUNC_DEF("getOwnPropertyDescriptors", 1,
...
const JSCFunctionListEntry js_object_proto_funcs[] = {
    ...
    JS_CGETSET_DEF("__proto__", js_object_get___proto__,
