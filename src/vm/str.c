...
JSValue string_buffer_end(StringBuffer *s) {
  ...
  if (s->len < s->size) {
    ...
    str = js_realloc_rt(s->ctx->rt, str,
                        sizeof(JSString) + (s->len << s->is_wide_char) + 1 -
                            s->is_wide_char);
    if (str == NULL)
      str = s->str;
    s->str = str;
...
static JSValue JS_ConcatString1(JSContext *ctx, const JSString *p1,
                                const JSString *p2) {
  JSString *p;
  uint32_t len;
  int is_wide_char;
...
static inline uint32_t hash_string8(const uint8_t *str, size_t len,
                                    uint32_t h) {
  size_t i;
...
static inline uint32_t hash_string16(const uint16_t *str, size_t len,
                                     uint32_t h) {
  size_t i;
...
JSAtom __JS_NewAtom(JSRuntime *rt, JSString *str, int atom_type) {
  ...
  if (rt->atom_free_index == 0) {
    ...
    new_array = js_realloc_rt(
        rt, rt->atom_array,
        sizeof(*new_array) * new_size); // NOLINT(bugprone-sizeof-expression)
    if (!new_array)
      goto fail;
  ...
  if (str) {
    if (str->atom_type == 0) {
      ...
      p = js_malloc_rt(rt, sizeof(JSString) + (str->len << str->is_wide_char) +
                               1 - str->is_wide_char);
      if (unlikely(!p))
        goto fail;
...
const char *JS_AtomGetStr(JSContext *ctx, char *buf, int buf_size,
                          JSAtom atom) {
  return JS_AtomGetStrRT(ctx->rt, buf, buf_size, atom);
}

static JSValue __JS_AtomToValue(JSContext *ctx, JSAtom atom,
                                BOOL force_string) {
