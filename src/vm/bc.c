...
static void js_object_list_init(JSObjectList *s) { memset(s, 0, sizeof(*s)); }
...
static int js_object_list_add(JSContext *ctx, JSObjectList *s, JSObject *obj) {
  ...
  if (js_resize_array(ctx, (void *)&s->object_tab, sizeof(s->object_tab[0]),
...
static void bc_put_u8(BCWriterState *s, uint8_t v) { dbuf_putc(&s->dbuf, v); }
...
static int bc_atom_to_idx(BCWriterState *s, uint32_t *pres, JSAtom atom) {
  ...
  if (js_resize_array(s->ctx, (void **)&s->idx_to_atom,
                      sizeof(s->idx_to_atom[0]), &s->idx_to_atom_size,
...
static int JS_WriteFunctionBytecode(BCWriterState *s, const uint8_t *bc_buf1,
                                    int bc_len) {
  int pos, len, op;
  JSAtom atom;
  uint8_t *bc_buf;
...
static JSValue JS_ReadBigNum(BCReaderState *s, int tag) {
  ...
  /* mantissa */
  if (a->expn != BF_EXP_ZERO && a->expn != BF_EXP_INF &&
      a->expn != BF_EXP_NAN) {
    if (bc_get_leb128(s, &len))
      goto fail;
...
static int BC_add_object_ref1(BCReaderState *s, JSObject *p) {
  if (s->allow_reference) {
    if (js_resize_array(s->ctx, (void *)&s->objects, sizeof(s->objects[0]),
...
static JSValue JS_ReadModule(BCReaderState *s) {
  ...
  if (m->req_module_entries_count != 0) {
    ...
    m->req_module_entries = js_mallocz(ctx, sizeof(m->req_module_entries[0]) *
  ...
  if (m->star_export_entries_count != 0) {
    ...
    m->star_export_entries = js_mallocz(ctx, sizeof(m->star_export_entries[0]) *
...
static JSValue JS_ReadSharedArrayBuffer(BCReaderState *s) {
  ...
  obj = js_array_buffer_constructor3(ctx, JS_UNDEFINED, byte_length,
                                     JS_CLASS_SHARED_ARRAY_BUFFER, data_ptr,
                                     NULL, NULL, FALSE);
  if (JS_IsException(obj))
    goto fail;
  if (BC_add_object_ref(s, obj))
