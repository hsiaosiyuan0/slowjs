...
function test_file1() {
	...
	assert(ret === size);
	for (i = 0; i < size; i++) assert(buf[i] === str.charCodeAt(i));
...
function test_file2() {
	...
	str = "hello world\n";
	size = str.length;
	for (i = 0; i < size; i++) f.putByte(str.charCodeAt(i));
	f.seek(0, std.SEEK_SET);
	for (i = 0; i < size; i++) {
...
function test_os() {
	...
	// skip `isatty` test when running in Github Runner
	...
	for (i = 0; i < buf.length; i++) buf[i] = i;
	...
	for (i = 0; i < buf.length; i++) assert(buf[i] == buf2[i]);
...
function test_timer() {
	...
	th = [];
	for (i = 0; i < 3; i++) th[i] = os.setTimeout(function () {}, 1000);
	for (i = 0; i < 3; i++) os.clearTimeout(th[i]);
}
