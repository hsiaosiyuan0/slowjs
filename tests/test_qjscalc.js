"use math"; // will change the precedence of the operator `^`
"use strict";
...
function test_modulo() {
  ...
  // explicit parentheses is required since the precedence of the operator `^` was
  // changed by the directive `"use math"`
...
function test_series() {
  ...
  assert(
    sin(X + O(X ^ 6)) == X - (1 / 6) * (X ^ 3) + (1 / 120) * (X ^ 5) + O(X ^ 6)
  );
  assert(
    cos(X + O(X ^ 6)) == 1 - (1 / 2) * (X ^ 2) + (1 / 24) * (X ^ 4) + O(X ^ 6)
  );
  assert(
    tan(X + O(X ^ 8)) ==
      X +
...
function test_trig() {
  ...
  assert_eq(
    (2 + 0.5 * I) ^ (1.1 - 0.5 * I),
    2.494363021357619 - 0.23076804554558092 * I
  );
